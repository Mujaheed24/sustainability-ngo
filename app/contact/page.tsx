      ...
      <section className="section-padding" style={{ background: '#f0faf1' }}>
        <div className="container-custom">
          <div style={{ display: 'grid', gap: '2rem', alignItems: 'start' }} className="contact-grid">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[
                ...
                <div key={item.title} style={{ background: '#ffffff', borderRadius: '1.125rem', padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                  <div style={{ background: '#f0faf1', borderRadius: '0.75rem', padding: '0.75rem', flexShrink: 0 }}>{item.icon}</div>
                  <div>
                    <h3 style={{ fontWeight: 700, color: '#0d1f2d', marginBottom: '2px', fontSize: '0.95rem' }}>{item.title}</h3>
                    {item.lines.map(line => <p key={line} style={{ color: '#1a5c2a', fontWeight: 600, margin: '0 0 2px', fontSize: '0.9rem' }}>{line}</p>)}
              ...
              {/* Social links */}
              <div style={{ background: '#ffffff', borderRadius: '1.125rem', padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                <h3 style={{ fontWeight: 700, color: '#0d1f2d', marginBottom: '1rem', fontSize: '0.95rem' }}>Follow Us</h3>
                <div style={{ display: 'flex', gap: '10px' }}>
                  {socialLinks.map((s) => (
                    <a
                      ...
                      onMouseEnter={e => { e.currentTarget.style.background = '#1a5c2a'; e.currentTarget.style.color = '#ffffff'; }}
                      onMouseLeave={e => { e.currentTarget.style.background = '#f0faf1'; e.currentTarget.style.color = '#1a5c2a'; }}
                    >
                      {s.icon}
                    </a>
                  ))}
            ...
            {/* Right — form */}
            <div style={{ background: '#ffffff', borderRadius: '1.5rem', boxShadow: '0 4px 20px rgba(0,0,0,0.07)', padding: '2.25rem' }}>
              {submitted ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '3rem 1rem', gap: '1rem' }}>
                  <CheckCircle style={{ width: '60px', height: '60px', color: '#1a5c2a' }} />
                  <h2 style={{ fontSize: '1.5rem', fontWeight: 900, color: '#0d1f2d', margin: 0 }}>Message Sent!</h2>
                  <p style={{ color: '#6b7280', maxWidth: '300px', lineHeight: 1.65, margin: 0 }}>
                    Thank you for reaching out. We'll get back to you within 1–2 business days.
                  </p>
                  <button
                    onClick={() => setSubmitted(false)}
                    style={{ background: 'none', border: 'none', color: '#1a5c2a', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', textDecoration: 'underline', marginTop: '0.5rem' }}
                  >
                ...
                <>
                  <h2 style={{ fontSize: '1.3rem', fontWeight: 900, color: '#1a5c2a', marginBottom: '1.5rem' }}>Send Us a Message</h2>
                  <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    <div style={{ display: 'grid', gap: '1.25rem' }} className="form-row">
                      <div>
                        ...
                        <input type="text" name="name" required value={formData.name} onChange={handleChange} placeholder="Your full name" className="field" />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, color: '#374151', marginBottom: '6px' }}>Email Address *</label>
                        <input type="email" name="email" required value={formData.email} onChange={handleChange} placeholder="you@example.com" className="field" />
                      </div>
                    </div>

                    <div>
                      <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, color: '#374151', marginBottom: '6px' }}>Subject *</label>
                      <select name="subject" required value={formData.subject} onChange={handleChange} className="field">
                    ...
                    <div>
                      ...
                      <textarea name="message" required rows={6} value={formData.message} onChange={handleChange} placeholder="Tell us how we can help…" className="field" style={{ resize: 'none' }} />
                    </div>
                    ...
                    <button
                      ...
                      disabled={loading}
                      style={{
                        background: loading ? '#6b7280' : '#1a5c2a',
                        color: '#ffffff',
                        fontWeight: 700,
                        fontSize: '1rem',
                        padding: '1rem',
